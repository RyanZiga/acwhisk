...
// Removed bucket creation import since buckets already exist
...
export function MediaUpload({ 
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      // First try direct Supabase upload as fallback
      const fileExtension = file.name.split('.').pop()
      const fileName = `${session.user.id}/${Date.now()}-${Math.random().toString(36).substring(7)}.${fileExtension}`
      ...
      if (response.ok) {
        ...
        console.log('Using existing bucket for fallback upload:', bucketName)
        ...
        if (uploadError) {
          ...
          if (uploadError.message.includes('Bucket not found')) {
            console.log('Bucket not found in fallback, creating and retrying...')
            ...
            if (bucketCreated) {
              ...
              throw new Error('Failed to create bucket for upload')
            }
          } else {
