...
app.post('/make-server-cfac176d/upload/:bucket', async (c) => {
  try {
    ...
    const fileName = `${user.id}/${isVideo ? 'videos' : 'images'}/${timestamp}-${randomId}.${fileExtension}`
    ...
    // Create signed URL with appropriate expiry
    const urlExpiry = isVideo ? 7 * 24 * 3600 : 24 * 3600 // 7 days for videos, 24 hours for images
    const { data: signedUrlData, error: urlError } = await supabase.storage
      .from(bucketName)
